...
import Contacts from "./whatsapp/Contacts";
import ChatBox from "./whatsapp/ChatBox";
import { ChatBoxVariant } from "./whatsapp/types/chatTypes";
import { useEffect, useRef, useState } from "react";
import { SnackbarProvider } from "notistack";
import ClearIcon from '@mui/icons-material/Clear';
import { useChatStore as WhatsappChatStore } from "./whatsapp/store/chatStore";

export default function GetWhatsappChat() {
    const cardRef = useRef<HTMLDivElement | null>(null);
    const positionRef = useRef({ x: 1000, y: 0 });
    const offsetRef = useRef({ x: 0, y: 0 });
    const draggingRef = useRef(false);
    const scale = 0.7;
    const { setChat: setWhatsappChat, width, height, setWidth, setHeight } = WhatsappChatStore(state => state);
    ...
    const onClearIconWhatsapp = (e: any) => {
        e.stopPropagation();
        setWidth("0");
        setHeight("0");
            ...
            >
                <Box
                    ...
                    <Stack justifyContent="space-between" direction="row" alignItems="center">
                        ...
                        <IconButton onClick={onClearIconWhatsapp} >
                            <ClearIcon />
                        </IconButton>
                    </Stack>
                </Box>
