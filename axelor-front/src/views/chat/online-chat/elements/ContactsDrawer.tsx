    ...
    const intervalEvent = setTimeout(() => {
      if (input.length > 0 && selectedContactGroup === 0) {
        if (!isNaN(parseInt(input[0]))) {
          ...
          sendEvent({
            event: "searchClients",
            data: { currentUserId, appealid: parseInt(input) },
          });
        }
        if (isNaN(parseInt(input[0]))) {
          setSearchLoading(true);
                ...
                {contacts && department && contacts[department.id] && (
                  <List sx={{ maxHeight: "200px", overflow: "auto" }}>
                    {contacts[department.id] &&
                      contacts[department.id].map((contact: any) => {
                        return (
                          <ListItemButton
                            key={contact.id}
                            sx={{ paddingLeft: "40px" }}
          ...
          clients?.map((client) => {
            return (
              <ListItemButton key={client.id} onClick={() => onClickListClient({ client, newOpen: false })}>
                ...
                {!client["client.id"] && (
                  <ListItemText primary={client.name} secondary={client.id} sx={{ marginLeft: "10px" }} />
                )}
              </ListItemButton>
            );
          })}
